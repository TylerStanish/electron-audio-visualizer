...
export function transformToVisualBins(array) {
  var newArray = new Uint8Array(NUM_BARS);
  for (var i = 0; i < NUM_BARS; i++) {
    var bin = Math.pow(i / NUM_BARS, spectrumScale) * (spectrumEnd - spectrumStart) + spectrumStart;
...
export function getTransformedSpectrum(array) {
...
function tailTransform(array) {
  ...
  for (var i = 0; i < NUM_BARS; i++) {
    ...
    } else if (NUM_BARS - i <= tailMargin) {
      value *= tailMarginSlope * Math.pow(NUM_BARS - i, marginDecay) + minMarginWeight;
    }
    values[i] = value;
  }
  return values;
...
function exponentialTransform(array) {
  ...
  for (var i = 0; i < array.length; i++) {
    var exp = (spectrumMaxExponent - spectrumMinExponent) * (1 - Math.pow(i / NUM_BARS, spectrumExponentScale)) + spectrumMinExponent;
    newArr[i] = Math.max(Math.pow(array[i] / spectrumHeight, exp) * spectrumHeight, 0);
  }
  return newArr;
}
